  ...
  try {
    ...
    if (isWindows()) {
      return {
        success: false,
        message:
          `输入文本失败：Windows 文本注入未成功。请确认目标窗口已聚焦、Claude Code 与目标应用权限级别一致，并且系统可用 powershell.exe。原始错误: ${message}`,
      }
    }
    if (isMacOS()) {
      ...
      if (
        ...
        return {
          success: false,
          message:
            '输入文本失败：缺少 macOS 自动化/辅助功能权限。请在 系统设置 -> 隐私与安全性 中为 Claude Code 授权「辅助功能」和「自动化 -> System Events」，然后重启 Claude Code。',
        }
      }
    }
